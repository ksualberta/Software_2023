...
#Max and Min Values for Rover Control 

Front_Left = 0
Front_Left_Max = 0.523
Front_Left_Min = -0.523

Front_Right = 1
Front_Right_Max = 0.523
Front_Right_Min = -0.523
...
Back_Right = 3
Back_Right_Max = 0.523
Back_Right_Min = -0.523
...
class RoverControl(Node):
    ...
    def __init__(self):
        ...
        self.Front_Left = 0
        self.Front_Left_Max = 0.523
        self.Front_Left_Min = -0.523
        self.Front_Left_offset = 0

        self.Front_Right = 1
        self.Front_Right_Max = 0.523
        self.Front_Right_Min = -0.523
        self.Front_Right_offset = 0


        self.Back_Left = 2
        self.Back_Left_Max = 0.523
        self.Back_Left_Min = -0.523
        self.Back_Left_offset = 0

        self.Back_Right = 3
        self.Back_Right_Max = 0.523
        self.Back_Right_Min = -0.523
        self.Back_Right_offset = 0
    ...
    def JoystickMsg(self, msg):
        ...
        print(self.joystick_msg_timestamp)
    ...
    def rover_main_control(self):
        ...
        self.steer_cmds_msg.data = [0.0 + self.Front_Left_offset, 0.0 + self.Front_Right_offset, 0.0 + self.Back_Left_offset, 0.0 + self.Back_Right_offset]

        self.keyboard_input()

        if self.joystick_msg.buttons[RIGHT_BUMPER] or self.joystick_msg.buttons[LEFT_BUMPER]:
            pass
        #DISABLED DO NOT USEE!!!!!!!!!!!
    ...
    def steer_motor_conv_unlocked(self):
        ...
        if self.joystick_msg.axes[LEFT_STICK_X] > threshold or self.joystick_msg.axes[LEFT_STICK_X] < -threshold:
            ...
            self.steer_cmds_msg.data[0] = self.steering_map(self.joystick_msg.axes[LEFT_STICK_X], self.Front_Left_Max, self.Front_Left_Min ) + self.Front_Left_offset
            self.steer_cmds_msg.data[1] = self.steering_map(self.joystick_msg.axes[LEFT_STICK_X], self.Front_Right_Max, self.Front_Right_Min) + self.Front_Right_offset
        

        if self.joystick_msg.axes[RIGHT_STICK_X] > threshold or self.joystick_msg.axes[RIGHT_STICK_X] < -threshold:

            self.steer_cmds_msg.data[2] = -(self.steering_map(self.joystick_msg.axes[RIGHT_STICK_X], self.Back_Left_Max, self.Back_Left_Min)) + self.Back_Left_offset
            self.steer_cmds_msg.data[3] = -(self.steering_map(self.joystick_msg.axes[RIGHT_STICK_X], self.Back_Left_Max, self.Back_Left_Min)) + self.Back_Right_offset
    ...
    def steer_motor_conv_locked(self):
        ...
        if self.joystick_msg.axes[LEFT_STICK_X] > threshold or self.joystick_msg.axes[LEFT_STICK_X] < -threshold:
            ...
            self.steer_cmds_msg.data[0] = self.steering_map(self.joystick_msg.axes[LEFT_STICK_X], self.Front_Left_Max, self.Front_Left_Min) + self.Front_Left_offset
            self.steer_cmds_msg.data[1] = self.steering_map(self.joystick_msg.axes[LEFT_STICK_X], self.Front_Right_Max, self.Front_Right_Min) + self.Front_Right_offset
            self.steer_cmds_msg.data[2] = -(self.steering_map(self.joystick_msg.axes[LEFT_STICK_X], self.Back_Left_Max, self.Back_Left_Min)) + self.Back_Left_offset
            self.steer_cmds_msg.data[3] = -(self.steering_map(self.joystick_msg.axes[LEFT_STICK_X], self.Back_Right_Max, self.Back_Right_Min)) + self.Back_Right_offset
    ...
    def steering_map(self, value,  max_value, min_value):

        new_value = min_value + (value -(-1))*((max_value-min_value)/(1-(-1)))
    ...
    def keyboard_input(self):
        ...
        if keys[pygame.K_1]:
            ...
            self.Front_Left_offset -= 0.02
            
        if keys[pygame.K_2]:
            self.Front_Left_offset += 0.02
            
        if keys[pygame.K_3]:
            self.Front_Right_offset -= 0.02
            
        if keys[pygame.K_4]:
            self.Front_Right_offset += 0.02
            
        if keys[pygame.K_5]:
            self.Back_Left_offset += 0.02
            
        if keys[pygame.K_6]:
            self.Back_Left_offset -= 0.02
            
        if keys[pygame.K_7]:
            self.Back_Right_offset -= 0.02
           
        if keys[pygame.K_8]:  # Assuming you meant 8 for increasing Back_Right
            self.Back_Left_offset += 0.02
